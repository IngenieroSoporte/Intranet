...
class EmpleosController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = Empleo::with(['salario'])->select(sprintf('%s.*', (new Empleo())->table));
            ...
            $table->addColumn('salario_nuevo', function ($row) {
                return $row->salario ? $row->salario->nuevo : '';
            });

            $table->editColumn('publicar', function ($row) {
                return $row->publicar ? Empleo::PUBLICAR_RADIO[$row->publicar] : '';
            });

            $table->rawColumns(['actions', 'placeholder', 'salario']);
    ...
    public function create()
    {
        ...
        $salarios = Salario::pluck('nuevo', 'id')->prepend(trans('global.pleaseSelect'), '');

        return view('admin.empleos.create', compact('salarios'));
    }
    ...
    public function edit(Empleo $empleo)
    {
        ...
        return view('admin.empleos.edit', compact('empleo', 'salarios'));
    }
    ...
    public function show(Empleo $empleo)
    {
        ...
        $empleo->load('salario', 'vacanteALaQueSePostulaCandidatos');
