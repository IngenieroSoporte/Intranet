...
class DocumentosCandidatosController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $table->editColumn('certificaciones', function ($row) {
                if (!$row->certificaciones) {
                    return '';
                }
                $links = [];
                foreach ($row->certificaciones as $media) {
                    $links[] = '<a href="' . $media->getUrl() . '" target="_blank">' . trans('global.downloadFile') . '</a>';
                }
            ...
            $table->rawColumns(['actions', 'placeholder', 'fotocopia_de_la_cedula', 'acta_de_grado', 'certificaciones']);
