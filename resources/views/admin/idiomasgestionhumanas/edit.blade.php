    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.idiomasgestionhumanas.update", [$idiomasgestionhumana->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nivel') ? 'is-invalid' : '' }}" type="text" name="nivel" id="nivel" value="{{ old('nivel', $idiomasgestionhumana->nivel) }}">
                @if($errors->has('nivel'))
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Idiomasgestionhumana::CETIFICACION_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('cetificacion') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="cetificacion_{{ $key }}" name="cetificacion" value="{{ $key }}" {{ old('cetificacion', $idiomasgestionhumana->cetificacion) === (string) $key ? 'checked' : '' }}>
