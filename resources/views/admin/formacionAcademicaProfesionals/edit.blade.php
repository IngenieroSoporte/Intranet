...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.formacionAcademicaProfesional.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.formacion-academica-profesionals.update", [$formacionAcademicaProfesional->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="titulo">{{ trans('cruds.formacionAcademicaProfesional.fields.titulo') }}</label>
                <input class="form-control {{ $errors->has('titulo') ? 'is-invalid' : '' }}" type="text" name="titulo" id="titulo" value="{{ old('titulo', $formacionAcademicaProfesional->titulo) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('numero_de_semestres') ? 'is-invalid' : '' }}" type="number" name="numero_de_semestres" id="numero_de_semestres" value="{{ old('numero_de_semestres', $formacionAcademicaProfesional->numero_de_semestres) }}" step="1">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\FormacionAcademicaProfesional::GRADUADO_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('graduado') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="graduado_{{ $key }}" name="graduado" value="{{ $key }}" {{ old('graduado', $formacionAcademicaProfesional->graduado) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('fecha_de_graduacion') ? 'is-invalid' : '' }}" type="text" name="fecha_de_graduacion" id="fecha_de_graduacion" value="{{ old('fecha_de_graduacion', $formacionAcademicaProfesional->fecha_de_graduacion) }}">
