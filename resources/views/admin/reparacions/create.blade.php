...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.reparacion.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.reparacions.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('activo') ? 'is-invalid' : '' }}" name="activo_id" id="activo_id" required>
                    @foreach($activos as $id => $entry)
                        <option value="{{ $id }}" {{ old('activo_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label class="required" for="descripcion">{{ trans('cruds.reparacion.fields.descripcion') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.reparacion.fields.descripcion_helper') }}</span>
            </div>
            <div class="form-group">
