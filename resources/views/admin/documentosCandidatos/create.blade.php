...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.documentosCandidato.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.documentos-candidatos.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="no_de_cedula">{{ trans('cruds.documentosCandidato.fields.no_de_cedula') }}</label>
                <input class="form-control {{ $errors->has('no_de_cedula') ? 'is-invalid' : '' }}" type="text" name="no_de_cedula" id="no_de_cedula" value="{{ old('no_de_cedula', '') }}" required>
            ...
            <div class="form-group">
                <label for="certificaciones">{{ trans('cruds.documentosCandidato.fields.certificaciones') }}</label>
                <div class="needsclick dropzone {{ $errors->has('certificaciones') ? 'is-invalid' : '' }}" id="certificaciones-dropzone">
                </div>
                @if($errors->has('certificaciones'))
                    <span class="text-danger">{{ $errors->first('certificaciones') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.documentosCandidato.fields.certificaciones_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    var uploadedCertificacionesMap = {}
Dropzone.options.certificacionesDropzone = {
    url: '{{ route('admin.documentos-candidatos.storeMedia') }}',
    maxFilesize: 20, // MB
    addRemoveLinks: true,
    headers: {
    ...
    success: function (file, response) {
      $('form').append('<input type="hidden" name="certificaciones[]" value="' + response.name + '">')
      uploadedCertificacionesMap[file.name] = response.name
    },
    removedfile: function (file) {
      file.previewElement.remove()
      var name = ''
      if (typeof file.file_name !== 'undefined') {
        ...
        name = uploadedCertificacionesMap[file.name]
      }
      $('form').find('input[name="certificaciones[]"][value="' + name + '"]').remove()
    },
    init: function () {
@if(isset($documentosCandidato) && $documentosCandidato->certificaciones)
          var files =
            {!! json_encode($documentosCandidato->certificaciones) !!}
              for (var i in files) {
              var file = files[i]
              this.options.addedfile.call(this, file)
              file.previewElement.classList.add('dz-complete')
              $('form').append('<input type="hidden" name="certificaciones[]" value="' + file.file_name + '">')
            }
@endif
    },
     error: function (file, response) {
