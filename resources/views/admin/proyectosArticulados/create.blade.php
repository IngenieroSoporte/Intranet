    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.proyectos-articulados.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('palabras_clave') ? 'is-invalid' : '' }}" type="text" name="palabras_clave" id="palabras_clave" value="{{ old('palabras_clave', '') }}" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ProyectosArticulado::PUBLICAR_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('publicar') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="publicar_{{ $key }}" name="publicar" value="{{ $key }}" {{ old('publicar', '1') === (string) $key ? 'checked' : '' }}>
