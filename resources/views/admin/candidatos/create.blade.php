...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.candidato.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.candidatos.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('vacante_a_la_que_se_postula') ? 'is-invalid' : '' }}" name="vacante_a_la_que_se_postula_id" id="vacante_a_la_que_se_postula_id">
                    @foreach($vacante_a_la_que_se_postulas as $id => $entry)
                        <option value="{{ $id }}" {{ old('vacante_a_la_que_se_postula_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('media') ? 'is-invalid' : '' }}" type="text" name="media" id="media" value="{{ old('media', '') }}">
                @if($errors->has('media'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('titulo_obtenido') ? 'is-invalid' : '' }}" type="text" name="titulo_obtenido" id="titulo_obtenido" value="{{ old('titulo_obtenido', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('formacion_academica_profesionals') ? 'is-invalid' : '' }}" name="formacion_academica_profesionals[]" id="formacion_academica_profesionals" multiple>
                    @foreach($formacion_academica_profesionals as $id => $formacion_academica_profesional)
                        <option value="{{ $id }}" {{ in_array($id, old('formacion_academica_profesionals', [])) ? 'selected' : '' }}>{{ $formacion_academica_profesional }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('idiomas') ? 'is-invalid' : '' }}" name="idiomas[]" id="idiomas" multiple>
                    @foreach($idiomas as $id => $idioma)
                        <option value="{{ $id }}" {{ in_array($id, old('idiomas', [])) ? 'selected' : '' }}>{{ $idioma }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('ofimaticas') ? 'is-invalid' : '' }}" name="ofimaticas[]" id="ofimaticas" multiple>
                    @foreach($ofimaticas as $id => $ofimatica)
                        <option value="{{ $id }}" {{ in_array($id, old('ofimaticas', [])) ? 'selected' : '' }}>{{ $ofimatica }}</option>
                    @endforeach
                </select>
